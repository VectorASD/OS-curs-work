...
void DynampicArrayTags(int tag, text* name, int* type) {
    ...
    case 0x6ffffdf9: n = "DT_PLTPADSZ"; t = 1; break;
    ...
    case 0x7ffffffe: n = "DT_USED"; t = 1; break;
...
# 569 "lab6.c"
bool is64, my_prog_little, elf_prog_little = 1;
...
int startswith(text str, text begin) {
    int pos = 0;
    char c;
    while (c = begin[pos]) if (str[pos++] != c) return 0;
    return 1;
}
...
# 913 "lab6.c"
    ...
    elf_header header;
    reader(data_p, "IBBBBBppppppp", &header);
    ...
    reader(data_p, "HHIxxxIHHHHHH", &header.e_type);
...
void* handler(void *sock_v) {
    ...
    while (1) {
        FILE *file;
        if ((file = fopen("resultator.txt", "w")) == 
# 1117 "lab6.c" 3 4
                                                    ((void *)0)
# 1117 "lab6.c"
                                                        ) {
...
# 1142 "lab6.c" 3 4
                           (-1)
# 1142 "lab6.c"
                              ) ({ post_str(sock, "error 22.) Не удалось закрыть файл результата\n", 0); continue; });

        struct stat st;
        if (stat("resultator.txt", &st) != 0) ({ post_str(sock, "error 23.) Не удалось узнать размер файла результата\n", 0); continue; });
        size = st.st_size;
        byte* res = (byte*) malloc(size);
        if ((file = fopen("resultator.txt", "r")) == 
# 1148 "lab6.c" 3 4
                                                    ((void *)0)
# 1148 "lab6.c"
                                                        ) ({ free(res); post_str(sock, "error 24.) Не удалось открыть файл результата на чтение\n", 0); continue; });
        if (fread(res, 1, size, file) != size) ({ free(res); post_str(sock, "error 25.) Не удалось считать файл результата\n", 0); continue; });
        if (fclose(file) == 
# 1150 "lab6.c" 3 4
                           (-1)
# 1150 "lab6.c"
                              ) ({ free(res); post_str(sock, "error 26.) Не удалось закрыть файл результата\n", 0); continue; });
...
# 1199 "lab6.c"
    ...
    sockaddr_in_t addr = { .sin_family = 
# 1204 "lab6.c" 3 4
                                        2
# 1204 "lab6.c"
                                               , .sin_port = htons(port), .sin_addr = { .s_addr = s_addr } };
...
int main(int args, text argv[args]) {
    ...
    if (args > 1) {
        ...
        bool options[] = {1, 1, 1, 1, 1, 1, 1, 1, 1};
        for (int i = 0; i < 9; i++) {
            char c = argv[1][i];
            if (c == 0) break;
            options[i] = c != '0' && c != ' ' && c != '-';
